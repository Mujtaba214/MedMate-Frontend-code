...
export default function Reminders() {
  ...
  const fetchReminders = async () => {
    try {
      setLoading(true);
      const res = await fetch("https://med-mate-backend-code-mx4z.vercel.app/api/reminders", {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`https://med-mate-backend-code-mx4z.vercel.app/api/reminders/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
